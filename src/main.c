...
if((my_pl != (struct shaga_plugin *)1)){
	_db_print("Closing mysql handle\n");
        my_pl->mysqlClose();
        _db_print("Unloading shaga module\n");
        unloadShagaPlugin(my_pl);
        _db_print("Shaga module unloaded\n");
        xfree(my_pl);
}
        my_pl = loadShagaPlugin();
	        if((int)my_pl == 1) {
        _db_print("Something wrong with shaga module\n");
        _db_print("Are you connected to database?\n");
}
